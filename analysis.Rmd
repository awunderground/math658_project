...
kable(starting_variance,
      caption = "Results of initial allocation of (1, 1, 1, 1)",
...
kable(head(select(priority_values, -missing_prop, -agg_priority_value, -N), n = 10), 
      caption = "Priority values for $\\bar{y}_{str}$",
      col.names = c("Stratum", "n", "$s^2_h$", "$N_h$", "Priority value", "$\\hat{V}(\\bar{y}_h)$"),
      digits = 0)
...
### Variance of $\bar{y}_{str}$ with Exact Optimal Allocation at the $ith$ unit

```{r mean-priority-values-plot, fig.height = 3}
priority_values %>%
  mutate(x = row_number()) %>%
...
condition2 %>%
  kable(caption = "Calculated sample sizes for $\\hat{y}_h$ in each stratum",
        col.names = c("Stratum", "$s^2_h$", "Missing proportion", "$N_h$", "N", "n_h"))
...
By noting that $\hat{p}_h$ is a special case of $\bar{y}_h$, and that when $y_i \in \{0, 1\}$ for all $i$, we can rewrite $S^2_h$ as $S^2_h = \frac{N_h}{N_h - 1}p_h(1 - p_h)$, then it follows from our decomposition of $V(\bar{y}_{str})$ that 

$V(\hat{p}_{str}) = \sum_{h = 1}^H (\frac{N_h}{N})^2 V(\hat{p}_h)$

$V(\hat{p}_{str}) = \sum_{h = 1}^H \frac{N^2_h}{N^2} (\frac{(N_h - 1)S^2_h}{N_h} - \frac{S^2_h}{1 \cdot 2} - \frac{S^2_h}{2 \cdot 3} - ... - \frac{S^2_h}{n_h(n_h - 1)})$

$V(\hat{p}_{str}) = \sum_{h = 1}^H \frac{N^2_h}{N^2} (\frac{(N_h - 1)}{N_h}\frac{N_h p_h(1- p_h)}{(N_h - 1)} - \frac{N_hp_h(1 - p_h)}{(N_h - 1)(1 \cdot 2)} - \frac{N_hp_h(1 - p_h)}{(N_h - 1)(2 \cdot 3)} - ... - \frac{N_hp_h(1 - p_h)}{(N_h - 1)(n_h)(n_h - 1)})$

\hfill

$V(\hat{p}_{str}) = \sum_{h = 1}^H \frac{N^2_hp_h(1 - p_h)}{N^2}$

$- \frac{N^3_1 p_1(1 - p_1)}{N^2(N_1 - 1) \cdot 1 \cdot 2} - \frac{N^3_1 p_1(1 - p_1)}{N^2(N_1 - 1) \cdot 2 \cdot 3} - ... - \frac{N^3_1 p_1(1 - p_1)}{N^2(N_1 - 1) n_1(n_1 - 1)}$

...

$- \frac{N^3_h p_h(1 - p_h)}{N^2(N_h - 1) \cdot 1 \cdot 2} - \frac{N^3_h p_h(1 - p_h)}{N^2(N_h - 1) \cdot 2 \cdot 3} - ... - \frac{N^3_h p_h(1 - p_h)}{N^2(N_h - 1) n_h (n_h - 1)}$
...
4) Pick value (1) from the table and increase the associated stratum's sample size by 1, so that the updated allocation is $(n_{12}, n_{22}, ..., n_{H2})$, where exactly one of the $n_{h2}$'s is equal to 2 and the rest are equal to 1. Then, compute $V(\hat{p}_{str}|n_{12}, ..., n_{H2}) = V(\hat{p}_{str}|n_{11}, ..., n_{H1}) - \frac{(1)}{N^2}$ where "(1)" represents the largest value from the table. If $V(\hat{p}_{str}|N_{12}, ..., n_{H2}) \leq V_0$, then stop with $n_1 = n_{12}, ..., N_H = N_{H2}$. Otherwise, go to step 5. 

5) Pick value (2) from the table and increase the associated stratum's sample size by 1, so that the updated allocation is $(n_{13}, ..., n_{H3})$. Then compute $V(\hat{p}_{str}|n_{13}, ..., n_{H3}) = V(\hat{p}_{str}|n_{12}, ..., n_{H2}) - \frac{(2)}{N^2}$, where "(2)" represents the second value from the table. If $V(\hat{p}_{str}|n_{13}, ..., N_H \leq V_0$, then stop with $n_1 = n_{13}, ..., n_H = n_{H3}$. Otherwise, continue until step $j$, where $V(\hat{p}_{str}|n_{1j}, ..., n_{Hj}) \leq V_0$. The final allocation is $n_{1j}, ..., n_{Hj})$ and $n = n_{1j} + \cdot\cdot\cdot + n_{Hj}$.

```{r proportion-strata-s2}
# 
...
kable(head(select(priority_values, -agg_priority_value, -N), n = 10), 
      caption = "Priority values for $\\hat{p}_{str}$",
      col.names =c("Stratum", "n", "$s^2_h$", "$N_h$", "Priority value", "$\\hat{V}(\\hat{p}_h)$"),
      align = "l")
...
### Variance of $\hat{p}_{str}$ with Exact Optimal Allocation at the $ith$ unit

```{r proportion-priority-value-plot, fig.height = 3}
priority_values %>%
  mutate(x = row_number()) %>%
  ggplot(aes(x = x, y = variance)) +
  geom_line() +
...
condition4 %>%
  kable(caption = "Calculated sample sizes for $\\hat{p}_h$ in each stratum",
        col.names = c("Stratum", "$s^2_h$", "$N_h$", "N", "n_h"))
```
...
tibble(quadrant = condition1$stratum,
  ...
  kable(caption = "Recommended strata sizes across the four conditions",
        col.names = c("Quadrant", "Condition 1", "Condition 2", "Condition 3", "Condition 4"),
        digits = 2)
```
...
nh %>%
  kable(caption = "Maximum recommended strata sizes across the four conditions",
        col.names = c("Quadrant", "$n_h$"))
```

## 8. Sampling Plan

Using the calculated values for $n_h$ from above, we sampled $n_h$ observations from each quadrant in Washington, DC. R does this by assigning a random number between zero and one to each observation and then selecting the observations with random numbers less than the desired proportion of values for each stratum. R can sample by proportion or integer. 
...
survey %>%
  sample_n(10) %>%
  select(fulladdress, unitnum, res_type) %>%
  kable(caption = "First 10 addresses sampled",
        col.names = c("Address", "Unit number", "Residence type"))
```
...
svyby(~property_value,            # variable to estimate
  ...
  kable(digits = 0,
        caption = "Mean appraised property value and 95% confidence interval in each quadrant",
        col.names = c("Quadrant", "$\\bar{y}_h$", "C.I. Lower", "C.I. Upper"))
```
...
tibble(
  ...
  kable(caption = "Proportion of homes in zones that don't restrict multi-family housing and 95% confidence interval",
        col.names = c("$\\bar{y}_{str}$", "C.I. Lower", "C.I. Upper"), 
        digits = 3)
```
...
svyby(~multifamily,            # variable to estimate
  ...
  kable(digits = 3,
        caption = "Proportion of homes in zones that don't restrict multi-family housing and 95% confidence interval by quadrant",
        col.names = c("Quadrant", "$\\hat{p}_h$", "C.I. Lower", "C.I. Upper"))
```
...
1. Applications of probability sampling with low measurement error are more enjoyable than application with high measurement error or survey non response. 
2. Measurement error and survey non response are pressing issues in applications of probability sampling and warrant further exploration by the authors of this report. 
