...
```{r}
condition2 <- strata %>%
  mutate(n = (N * s_squared_h) / ((N - 1) * (50000 ^ 2 / qnorm(0.95)) + s_squared_h)) 
...
rm(pilot_sample, n_strata)
```

### Sample proportion within strata
...
```{r}
condition4 <- strata %>%
  mutate(n = (N * 0.25) / ((N - 1) * (0.1 ^ 2 / qnorm(0.95)) + 0.25)) 
