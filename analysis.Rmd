...
We were interested in estimating the sample mean of property values for all DC residences using stratification, the sample means of property values for all residences in each quadrant of DC, the sample proportion of residences in zones that don't restrict multi-family housing for all DC residences using stratifcation, and the sample proportions of residences in zones that don't restrcit multi-family housing. All four of these estimation processes required allocating a certain number of sample units to each stratum and each process has a different optimal allocation. 
...
$V(\hat{p}_{str}) = \sum_{h = 1}^H \frac{N^2_h}{N^2} (\frac{(N_hh - 1)S^2_h}{N_h} - \frac{S^2_h}{1 \cdot 2} - \frac{S^2_h}{2 \cdot 3} - ... - \frac{S^2_h}{n_h(n_h - 1)})$
...
$- \frac{N^3_1 p_1(1 - p_1)}{N^2(N_1 - 1) \cdot 1 \cdot 2} - \frac{N^3_1 p_1(1 - p_1)}{N^2(N_1 - 1) \cdot 2 \cdot 3} - ... - \frac{N^3_1 p_1(1 - p_1)}{N^2(N_1 - 1) n_h(n_h - 1)}$
