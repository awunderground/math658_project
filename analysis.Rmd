...
s2 <- pilot_sample %>%
  ...
  summarize(s2 = var(property_value, na.rm = TRUE), 
...
strata <- left_join(s2, Nh, by = "stratum")
...
priority_values <- n_strata %>%
  ...
  group_by(stratum) %>%
  mutate(priority_value = (N^2 * s2) / (n * lag(n)))
```


Step 3: $V(\bar{y}|1, 1, 1, 1) = \sum_{h = 1}^4 N_h(N_h - 1) S_h^2$

```{r}
# step 3  
starting_variance <- strata %>%
  mutate(N = N * (1 - missing_prop)) %>%
  mutate(strata_variance = N * (N - 1) * s2) %>%
